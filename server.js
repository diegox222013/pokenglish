...
const mons = { pikachu:{icon:'⚡',type:'Electric',hp:110,atk:22}, squirtle:{icon:'💧',type:'Water',hp:125,atk:20}, charmander:{icon:'🔥',type:'Fire',hp:105,atk:25}, bulbasaur:{icon:'🌿',type:'Grass',hp:120,atk:21}, pidgey:{icon:'🪽',type:'Flying',hp:100,atk:23}, raichu:{icon:'⚡',type:'Electric',hp:125,atk:27} };
...
const players = new Map(), battles = new Map(), invites = new Map(); let owner = null, started = true;
...
function wildBattle(s,p){if(battles.has(s.id))return;const w=wild[Math.floor(Math.random()*wild.length)],lv=clamp(p.level+Math.floor(Math.random()*5)-2,1,100),hp=w[3]+lv*7,q=question();battles.set(s.id,{kind:'wild',enemy:{name:w[0],icon:w[1],type:w[2],level:lv,maxHp:hp,hp,atk:w[4]+Math.floor(lv/3)},q,started:Date.now()});p.questionAt=Date.now();s.emit('battleStarted',{kind:'wild',enemy:battles.get(s.id).enemy,question:{q:q[0],a:q[1],c:q[2],hint:q[3]}})}
...
app.disable('x-powered-by');app.use((req,res,next)=>{res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('X-Frame-Options','SAMEORIGIN');next()});
app.get('/health',(req,res)=>res.json({ok:true,version:'SUPREME-STABLE',world:WORLD,players:players.size,started:true}));
app.get('/',(req,res)=>res.sendFile(path.join(publicDir,'game-v9.html')));app.use(express.static(publicDir));
io.on('connection',socket=>{
 socket.on('joinPlayer',(data={})=>{if(players.has(socket.id))return;const name=cleanName(data.name),old=profiles[name.toLowerCase()]||{},mon=old.pokemon||data.pokemon||'pikachu',base=mons[mon]||mons.pikachu,spawn=towns[0];if(!owner)owner=socket.id;const p={id:socket.id,name,pokemon:mon,x:spawn.x,y:spawn.y,level:clamp(Number(old.level)||1,1,100),xp:Number(old.xp)||0,tokens:Number(old.tokens)||0,wins:Number(old.wins)||0,streak:0,stats:old.stats||{hp:base.hp,hpMax:base.hp,attack:base.atk},inventory:old.inventory||{snack:3,boost:0,badge:0},quests:old.quests||{visited:['lumen'],bestStreak:0},lastMove:Date.now(),questionAt:0};p.stats.hp=Math.min(p.stats.hp,p.stats.hpMax);players.set(socket.id,p);socket.emit('serverReady',{id:socket.id,owner:socket.id===owner,world:WORLD,towns,regions,routes,grass,water,mountains});progress(socket,p);socket.emit('roomStarted');broadcast();io.emit('chatMessage',{name:'SYSTEM',text:p.name+' se unió a la aventura.'})});
 socket.on('playerMove',(d={})=>{const p=players.get(socket.id);if(!p)return;const nx=Number(d.x),ny=Number(d.y);if(!Number.isFinite(nx)||!Number.isFinite(ny))return;const now=Date.now(),dt=Math.min(.35,Math.max(.016,(now-p.lastMove)/1000));if(Math.hypot(nx-p.x,ny-p.y)>220*dt+25)return;p.x=clamp(nx,24,WORLD.w-24);p.y=clamp(ny,24,WORLD.h-24);p.lastMove=now;const t=nearestTown(p);if(t.d<280&&!p.quests.visited.includes(t.id)){p.quests.visited.push(t.id);saveProfile(p);socket.emit('questData',[{title:'Primer paso',desc:'Gana 1 combate',done:p.wins>=1},{title:'Explorador',desc:'Visita 3 pueblos',done:p.quests.visited.length>=3},{title:'Académico',desc:'Alcanza nivel 5',done:p.level>=5},{title:'Campeón',desc:'Gana 10 combates',done:p.wins>=10},{title:'Racha maestra',desc:'Consigue una racha de 5',done:p.quests.bestStreak>=5}])}broadcast()});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`PokéWords SUPREME STABLE listening on ${PORT}`));
